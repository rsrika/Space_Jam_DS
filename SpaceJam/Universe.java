...
public class Universe 
{
	...
	public static void readAndLoadUniverse(String fileName)
	{
		
		File f = new File("./src/input/"+fileName);
		try
		{
			...
			for(int i = 0; i< numParticles; i++)
			{
				if(br.ready())
				{
					...
					particles1[i] = particles1[i].buildParticleFromFile(lineSplit);
				}
			}
	...
	{
		readAndLoadUniverse("planets.txt");
		for(int i = 0; i<particles.length; i++)
		{
			if(particles[i]!= null)
			{
			...
			{
				System.out.println("particle not registered properly");
			}
		}
	}
